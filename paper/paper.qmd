...
running-head: '*R package `movetrack`*'
authors:
  - name: Georg Rüppel
...
affiliations:
  - id: 1
    ...
    department: Institute for Biology and Environmental Sciences
abstract: |
...
keywords: [animal movement, hidden Markov model, random walk, R package, telemetry]
...
![Basic model structure showing the hidden state process $\{S_t\}$ governing the real locations $z$ and the observed locations $y$ at time $t$. \label{fig:hmm}](files/hmm.svg)
...
![Influence of the input arguments `dTime` and `aRange` on the deviation between the estimated positions and the GPS track. \label{fig:deviation}](files/deviation.svg){width="360pt"}

![Estimated positions (diamonds) over time together with a part of the test flight's GPS track (line). GPS locations with detections by radio-telemetry stations are indicated by dots, longitudinal and latitudinal model uncertainties (90% HPDI) are indicated by ellipses. \label{fig:map}](files/map.svg){width="300pt"}

![The deviation between the estimated positions and the GPS track varied based on the aircraft's distance to the nearest Motus station and the number of antennas simultaneously receiving the signal. \label{fig:deviation_ant}](files/deviation_ant.svg){width="330pt"}
